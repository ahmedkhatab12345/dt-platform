...
@php($title = 'إضافة محور')
@php($pageHeading = 'إضافة محور')

@section('content')
<div class="bg-white shadow rounded-xl p-6 max-w-xl mx-auto">
  <h2 class="text-2xl font-bold text-gray-800 mb-6">إضافة محور</h2>
  ...
  <form method="POST" action="{{ route('pillars.store') }}" class="space-y-5">
    @csrf
    ...
    <div>
      ...
      <select name="perspective_id" required
        ...
        @foreach($perspectives as $p)
          <option value="{{ $p->id }}" {{ old('perspective_id')==$p->id?'selected':'' }}>
            {{ $p->name }}
          </option>
        @endforeach
      </select>
