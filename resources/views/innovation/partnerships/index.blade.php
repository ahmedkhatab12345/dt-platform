...
@php($title = 'الشراكات والاتفاقيات')
@php($pageHeading = 'إدارة الشراكات والاتفاقيات')
...
<div class="bg-white rounded-xl shadow p-6">
  ...
  <div class="flex items-center gap-4 mb-6">
    @can('create partnerships')
      <a href="{{ route('innovation.partnerships.create') }}"
  ...
  <form method="GET" action="{{ route('innovation.partnerships.index') }}" class="mb-4">
    <div class="flex flex-col md:flex-row items-start md:items-end gap-3">
      ...
      <div>
        ...
        <input id="q" type="text" name="q" value="{{ request('q') }}"
              placeholder="اسم الاتفاقية / التفاصيل..."
    ...
    <table class="w-full table-fixed border-collapse bg-white rounded-lg shadow text-sm">
      <thead>
        <tr class="bg-gray-100 text-gray-700 text-right uppercase text-xs">
          ...
          <th class="px-4 py-3">الشراكات / الاتفاقيات</th>
          ...
          <tr class="hover:bg-gray-50 align-top">
            ...
            <td class="px-4 py-3 text-center">
              <div class="flex justify-center gap-2">
                @can('update partnerships')
                  <a href="{{ route('innovation.partnerships.edit', $row) }}"
                    class="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600 text-xs">
                    تعديل
                  </a>
                @endcan

                @can('delete partnerships')
                  <form method="POST" action="{{ route('innovation.partnerships.destroy', $row) }}"
        ...
        @empty
          <tr>
            <td colspan="7" class="px-4 py-6 text-center text-gray-500 italic">
              لا توجد شراكات / اتفاقيات حتى الآن.
            </td>
          </tr>
        @endforelse
      </tbody>
