...
@php($title = 'الفاعليات والمبادرات')
@php($pageHeading = 'إدارة الفاعليات والمبادرات')
...
<div class="bg-white rounded-xl shadow p-6">
  ...
  <div class="flex items-center gap-4 mb-6">
    @can('create events')
      <a href="{{ route('innovation.events.create') }}"
  ...
  <form method="GET" action="{{ route('innovation.events.index') }}" class="mb-4">
    <div class="flex flex-col md:flex-row items-start md:items-end gap-3">
      ...
      <div>
        ...
        <input id="q" type="text" name="q" value="{{ request('q') }}"
              placeholder="اسم الفعالية / التفاصيل..."
    ...
    <table class="w-full table-fixed border-collapse bg-white rounded-lg shadow text-sm">
      <thead>
        <tr class="bg-gray-100 text-gray-700 text-right uppercase text-xs">
          ...
          <th class="px-4 py-3">الفاعليات / المبادرات</th>
          ...
          <tr class="hover:bg-gray-50 align-top">
            ...
            <td class="px-4 py-3 text-center">
              <div class="flex justify-center gap-2">
                @can('update events')
                  <a href="{{ route('innovation.events.edit', $row) }}"
                ...
                @can('delete events')
                  <form method="POST" action="{{ route('innovation.events.destroy', $row) }}"
        ...
        @empty
          <tr>
            <td colspan="7" class="px-4 py-6 text-center text-gray-500 italic">
              لا توجد فاعليات / مبادرات حتى الآن.
            </td>
          </tr>
        @endforelse
      </tbody>
