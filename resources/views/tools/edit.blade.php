...
@php($title = 'تعديل أداة')
@php($pageHeading = 'تعديل أداة')

@section('content')
<div class="bg-white shadow rounded-xl p-6 max-w-xl mx-auto">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">تعديل أداة</h2>

    <form method="POST" action="{{ route('tools.update', $tool) }}" class="space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="url" name="link" value="{{ old('link',$tool->link) }}"
                   class="w-full border-gray-300 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-indigo-500">
        </div>
