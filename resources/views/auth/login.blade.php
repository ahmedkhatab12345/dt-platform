  ...
  <div class="w-full max-w-[420px] rounded-3xl bg-white/80 backdrop-blur-md border border-white/50 shadow-2xl p-8 text-right">
    ...
    <form id="loginForm" method="POST" action="{{ route('login') }}" class="mt-6 space-y-5">
      ...
      <div>
        ...
        <div class="relative">
          <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="username"
            ...
            class="w-full pr-10 pl-4 py-2 rounded-xl border border-slate-300 bg-white/70 shadow-sm
                   focus:outline-none focus:ring-2 focus:ring-indigo-500 text-right">

          <!-- أيقونة البريد *على اليسار* -->
      ...
      <div>
        ...
        <div class="relative">
          <input id="password" type="password" name="password" required autocomplete="current-password"
            placeholder="••••••••"
            class="w-full pr-14 pl-10 py-2 rounded-xl border border-slate-300 bg-white/70 shadow-sm
                   focus:outline-none focus:ring-2 focus:ring-indigo-500 text-right">

          <!-- زر إظهار / إخفاء على اليسار -->
          <button type="button" id="togglePassword"
            class="absolute left-3 top-1/2 -translate-y-1/2 text-xs px-2 py-1 rounded-md
                   text-slate-600 hover:text-slate-800 bg-white/50">
            إظهار
          </button>
        </div>        
      ...
      <button id="submitBtn" type="submit"
        class="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 text-white
               font-medium py-2.5 shadow-lg hover:bg-indigo-700 hover:shadow-xl active:scale-[.99] transition">
